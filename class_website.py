...
class BaseHandler(tornado.web.RequestHandler):
    ...
    def get_current_user(self):
        if not self.get_cookie("webstore_cookie"):
            self.set_cookie("webstore_cookie", str(uuid.uuid4()), httponly=True, secure=True, samesite="Strict")
...
class DetailsHandler(BaseHandler):
    def get(self, slug):
        ...
        item_number = re.findall(r'\d+', tornado.escapt.xhtml_escape(slug))
        c = self.db.cursor()
        c.execute("SELECT * \
                   FROM products p \
        ...
        self.render("details.html",
                    product=tuple(temp),
                    sku=tornado.escape.xhtml_escapte(slug),
                    auth=authorized)
...
class CartHandler(BaseHandler):
    def get(self):
        ...
        c = self.db.cursor()
	query"""SELECT c.item,p.price,p.name,COUNT(*) AS quantity,SUM(p.price) AS subtotal,`options`,GROUP_CONCAT(c.id) AS `id`\
                   FROM cart c \
...
class AddToCartHandler(BaseHandler):
    def post(self):
        ...
        c = self.db.cursor()
        query="""INSERT INTO cart (id, user_cookie, item, options) \
                   VALUES (0, %s, %s, %s)"""
...
class WelcomeHandler(BaseHandler):
    def get(self, name):
	#Escape user input to prevent XSS and SSTI
	sanitized_name = tornado.escape.xhtml_escape(name)
        TEMPLATE = open("templates/welcome.html").read()
        template_data = TEMPLATE.replace("FOO",sanitized_name)
        t = tornado.template.Template(template_data)
        self.write(t.generate(name=sanitized_name))
...
class DirectoryTraversalHandler(BaseHandler):
    def get(self, slug):
# Define a base directory and sanitize the path t avoid directory traversal
        base_path = os.path.normpath(os.path.join(base_path, slug))
	#Ensure the request file is within the base directory
